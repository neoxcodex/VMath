...
namespace dim3 {
    ...
    inline float magSquared(Vec3 a){
        return dotProduct(a,a);
    }
    inline void vectorCout(Vec3 a){
    ...
    inline Vec3 scalarMult(Vec3 a, float scalar){
    ...
    inline float angleRAD(Vec3 a, Vec3 b, bool returnCosValue = false){
    ...
    inline float angleDEG(Vec3 a, Vec3 b){
        return angleRAD(a,b) * constants::INV_PI_180;
    }
    inline float scalarProject(Vec3 vecToProj, Vec3 vecToProjAlong){
        float magAlong = std::sqrt(magSquared(vecToProjAlong));
        return (dotProduct(vecToProjAlong, vecToProj) / magAlong);

    }
    inline Vec3 vectorProject(Vec3 vecToProj, Vec3 vecToProjAlong){
